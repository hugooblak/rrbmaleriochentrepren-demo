...
add_filter(
	...
	function ( $exporters ) {
		$exporters['rrbmaleriochentrepren-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'rrbmaleriochentrepren-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'rrbmaleriochentrepren-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'rrbmaleriochentrepren-leads',
						'group_label' => __( 'Quote requests', 'rrbmaleriochentrepren-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['rrbmaleriochentrepren-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'rrbmaleriochentrepren-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
