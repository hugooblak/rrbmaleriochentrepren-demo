...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'rrbmaleriochentrepren-leads' ),
					'singular_name' => __( 'Lead', 'rrbmaleriochentrepren-leads' ),
					'edit_item'     => __( 'Förfrågan', 'rrbmaleriochentrepren-leads' ),
					'search_items'  => __( 'Search leads', 'rrbmaleriochentrepren-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'rrbmaleriochentrepren-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'rrbmaleriochentrepren-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'rrbmaleriochentrepren-leads' ),
			'npl_phone'    => __( 'Phone', 'rrbmaleriochentrepren-leads' ),
			'npl_service'  => __( 'Needs', 'rrbmaleriochentrepren-leads' ),
			'npl_timeline' => __( 'When', 'rrbmaleriochentrepren-leads' ),
			'npl_zip'      => __( 'ZIP', 'rrbmaleriochentrepren-leads' ),
			'npl_source'   => __( 'Source', 'rrbmaleriochentrepren-leads' ),
			'npl_status'   => __( 'Status', 'rrbmaleriochentrepren-leads' ),
			'date'         => __( 'Received', 'rrbmaleriochentrepren-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'rrbmaleriochentrepren-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'rrbmaleriochentrepren-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'rrbmaleriochentrepren-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'rrbmaleriochentrepren-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'rrbmaleriochentrepren-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'rrbmaleriochentrepren-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'rrbmaleriochentrepren-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'rrbmaleriochentrepren-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'rrbmaleriochentrepren-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'rrbmaleriochentrepren-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'rrbmaleriochentrepren-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'rrbmaleriochentrepren-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
