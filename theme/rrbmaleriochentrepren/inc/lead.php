 ...
 * or photo. They call rr_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional RRB måleri & entreprenad AB.
 *
 ...
 * @return string
 */
function rr_sample_note() {
	return rr_lead( 'ar_riktig' ) ? '' : ' <span class="rr-sample">(platshållare)</span>';
}
 ...
 * @return string File name for rr_lead_img().
 */
function rr_lead_bild( $i ) {
	$bilder = (array) rr_lead( 'bilder', array() );
 ...
 * @return string
 */
function rr_ring_label() {
	$namn = rr_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, rr_lead( 'telefon' ) ) : 'Ring ' . rr_lead( 'telefon' );
}
